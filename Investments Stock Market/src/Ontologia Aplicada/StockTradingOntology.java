...
public class StockTradingOntology extends Ontology {
  ...
  public static final Integer BUY_QUANTITY               = 0;

  // AGENT ACTION - AGENTE VENDEDOR
  public static final String  SELL                       = "sell";
  public static final String  SELL_ITEM                  = "sell_item";
  public static final Integer SELL_QUANTITY              = 0;
  ...
  private StockTradingOntology() {
    ...
    try {
      ...
      cs.add(STOCK_AVAILABLE, (PrimitiveSchema) getSchema(BasicOntology.INTEGER));
      cs.add(STOCK_LAST_STOCK_PRICE, (PrimitiveSchema) getSchema(BasicOntology.FLOAT));
      cs.add(STOCK_YEAR, (PrimitiveSchema) getSchema(BasicOntology.INTEGER));
      cs.add(STOCK_STOCK, (PrimitiveSchema) getSchema(BasicOntology.INTEGER)); // TODO Ver o que é esta Variável em Empresa.java
      cs.add(STOCK_COMPANY_CAPITAL, (PrimitiveSchema) getSchema(BasicOntology.INTEGER));
      ...
      ps.add(TRANSACTION_COSTS_PRICE, getSchema(BasicOntology.FLOAT));
      ps.add(TRANSACTION_COSTS_QUANTITY, getSchema(BasicOntology.INTEGER));

      // Structure of the schema for the BidderAgent agent action
      AgentActionSchema asBidder = (AgentActionSchema) getSchema(BUY);
      asBidder.add(BUY_ITEM, (ConceptSchema) getSchema(STOCK), BUY_QUANTITY);

      // Structure of the schema for the SellerAgent agent action
      AgentActionSchema asSeller = (AgentActionSchema) getSchema(SELL);
      asSeller.add(SELL_ITEM, (ConceptSchema) getSchema(STOCK), BUY_QUANTITY);
    }
